...
*ᕤ•˙ ⃞✦ SCRIPT  : rizal-dev*
*ᕤ•˙ ⃞✦ BOT NAME : rizal-dev BOT*
*ᕤ•˙ ⃞✦ PENGGUNA NAMA : ${namabot}*
*ᕤ•˙ ⃞✦ BUY SC : 6288210180505*
*ᕤ•˙ ⃞✦ BUY BOT : 6288210180505*
...
case "sc" :
case "script": {
const text12 = `> UNTUK SCRIPT BISA LANGSUNG TANYA KE rizal-dev AJA KAK INI NOMORNYA 085926340758 BTW DIA JUALAN PANEL JUGA`
reply(text12)
}
break
case "yt": {
...
let barang = t[0];
let nominal = t[1];
reply(`> *TRANSAKSI BERHASIL RIZAL-DEV*
▬▭▬▭▬▭▬▭▬▬▭▬▭
...
*NOMINAL* : Rp${nominal}
*SISTEM :* DIRECT
> *SOSMED rizal-dev :* https://www.instagram.com/zal_x_u
▬▭▬▭▬▭▬▭▬▬▭▬▭
> *NOTE:* TERIMA KASIH TELAH ORDER DAN MEMPERCAYAI *rizal-dev* JANGAN LUPA ORDER LAGI YA🙏`)
}
        break
case 'proses':{
m.reply('*SIAP PESANAN ANDA AKAN KAMI PROSES JADI DI MOHON UNTUK MENUNGGU SEBENTAR YAH BANG*')
haikal.sendMessage(owner + "@s.whatsapp.net", { text: "BANG rizal-dev ADA YANG MESEN NIH CEPETAN PROSES NANTI BUYER NGAMOK", contextInfo:{ forwardingScore: 9999, isForwarded: true }})
}
break
case "formatneed": {
...
break
case "listpanel": {
const text12 = `*LIST HARGA PANEL RUN BOT BY rizal-dev⚡*
...
let groups = Object.entries(getGroups).slice(0).map((entry) => entry[1])
let anu = groups.map((v) => v.id)
let teks = `⬣ *LIST GROUP BY rizal-dev*\n\n`
for (let x of anu) {
let metadata2 = await haikal.groupMetadata(x)
...
case "listgroup":{
if (!isOwner) return (`> Ngapain ? Khusus rizal-dev Aja !!`)
let getGroups = await haikal.groupFetchAllParticipating()
let groups = Object.entries(getGroups).slice(0).map((entry) => entry[1])
let anu = groups.map((v) => v.id)
let hituet = 0
...
break
case "jpm": case "jpm2":{
if (!isOwner) return reply(`> Khusus Bang rizal-dev`)
...
reply("*SUCCESFUL BANG rizal-dev✅*")
}
break
case "pushkontakid":{
if (!isOwner) return reply(`> Khusus Bang rizal-dev`)
...
break
case "pushkontakgc":{
if (!isOwner) return reply(`> Khusus rizal-dev`)
...
Penggunaan:
${prefix + command} |nama pake garus lurus sebelum nama yah bang rizal-dev | yang kaya gitu`);
let nama = t[0];
let nominal = t[1];
        reply(mess.save)
...
await reply("> Laksanakan rizal-dev")
...
reply("> Succes rizal-dev!")
break
case "pushkontakgcjd":
if (!isOwner) return reply(`> Khusus rizal-dev`)
if (!isGroup) return reply(mess.only.group)
if (!text) return reply(`> Penggunaan Salah Silahkan Gunakan Command Seperti Ini\n${prefix+command} jeda|teks`)
await reply("> Laksanakan Bang rizal-dev")
...
if (!isGroup) return m.reply(`> Khusus Group Kontol`)
huhuhs = await haikal.sendMessage(m.chat, {
    text: `> *ID :* ${groupMetadata.id}\n\nJangan Lupa Di Salin Idnya Yah Bang rizal-dev*`
}, {quoted: m, ephemeralExpiration: 86400})
await sleep(1000) // (?); mengirim kontak seluruh member
reply
break
case 'savekontak': case 'svkontak':
if (!isOwner) return reply(`> Khusus rizal -dev Aja`)
if (!isGroup) return m.reply(`> Khusus Group Kontol`)
